...
export default function ReservationsPage() {
  ...
  const t = useTranslations('customerReservations')
  const tCommon = useTranslations('common')

  // Map table status to labels and colors
  const statusConfig = {
    available: { label: t('available'), color: 'bg-green-500', icon: CheckCircle },
    occupied: { label: t('inUse'), color: 'bg-red-500', icon: Users },
    hidden: { label: t('unavailable'), color: 'bg-gray-500', icon: XCircle }
  }
  ...
  return (
    <div className='container mx-auto px-4 py-6 max-w-4xl'>
      <div className='mb-8 flex items-center justify-between gap-3'>
        <div>
          <h1 className='text-3xl font-bold mb-2'>{t('myReservations')}</h1>
          <p className='text-muted-foreground'>{t('manage')}</p>
        </div>
        <Button onClick={() => setIsReserveOpen(true)}>{t('reserveTable')}</Button>
      </div>
          ...
          <Card>
            <CardContent className='pt-6'>
              <div className='text-center text-muted-foreground'>
                ...
                <p className='text-lg font-medium mb-2'>{t('noReservations')}</p>
                <p className='text-sm'>
                  {selectedTab === 'upcoming' && t('noUpcoming')}
                  {selectedTab === 'past' && t('noPast')}
                </p>
              </div>
              ...
              <Card key={reservation.id}>
                <CardHeader>
                  <div className='flex items-start justify-between'>
                    <div>
                      <CardTitle>{t('tableNumber')} {reservation.tableNumber}</CardTitle>
                      <CardDescription>
                  ...
                  <div className='space-y-4'>
                    <div className='grid md:grid-cols-2 gap-4'>
                      <div className='flex items-center gap-3'>
                        ...
                        <div>
                          <p className='font-medium'>{t('capacityLabel', { count: reservation.capacity })}</p>
                      ...
                      <div className='flex items-center gap-3'>
                        ...
                        <div>
                          ...
                          <p className='text-sm text-muted-foreground'>{t('locationTable')}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className='bg-muted/50 p-3 rounded-lg'>
                      <p className='text-sm'>
                        <span className='font-medium'>{t('customerType')}:</span>{' '}
                        {reservation.isCustomer ? t('member') : t('guest')}
                      </p>
                      {(reservation.customerId || reservation.guestId) && (
                        <p className='text-sm mt-1'>
                          <span className='font-medium'>{t('id')}:</span> {reservation.customerId || reservation.guestId}
                        </p>
                      )}
                    </div>
                    ...
                    {reservation.note && (
                      <div className='bg-muted p-3 rounded-lg'>
                        <p className='text-sm'>
                          <span className='font-medium'>{t('note')}:</span> {reservation.note}
                        </p>
                      </div>
                    )}
                    ...
                    <div className='flex gap-2'>
                      {canCancel && (
                        ...
                        >
                          ...
                          {cancelReservationMutation.isPending ? t('canceling') : t('cancelReservation')}
                        </Button>
                      )}
                      {canOrder && (
      ...
      <Dialog open={isReserveOpen} onOpenChange={setIsReserveOpen}>
        <DialogContent className='sm:max-w-[560px]'>
          <DialogHeader>
            <DialogTitle>{t('reserveTable')}</DialogTitle>

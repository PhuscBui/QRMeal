      ...
      <Card className='overflow-hidden shadow-md'>
        <CardHeader className='pb-3 border-b'>
          <div className='flex justify-between items-center'>
            <div>
              <CardTitle className='text-xl font-bold'>{t('tableNumber', { number: table.number })}</CardTitle>
              <p className='text-sm text-muted-foreground mt-1'>
                {t('guest')} <span className='font-medium text-foreground'>{guest.name}</span>
              </p>
            </div>
        ...
        <CardContent className='pt-4 space-y-5'>
          ...
          <div className='space-y-3'>
            ...
            <div className='flex items-start gap-3'>
              ...
              <div className='text-sm'>{t('people', { count: table.capacity })}</div>
            </div>
          ...
          <div className='bg-primary/10 p-4 rounded-lg text-center'>
            <div className='font-medium'>
              {t('table')} <span className='text-primary font-bold text-lg'>{table.number}</span>
            </div>
          </div>
        </CardContent>
        ...
        <CardFooter className='pt-2 pb-4'>
          ...
          >
            {cancelReservation.isPending ? t('canceling') : t('cancelBooking')}
          </Button>
        </CardFooter>
      </Card>
      ...
      <Dialog open={confirmDialogOpen} onOpenChange={setConfirmDialogOpen}>
        <DialogContent className='max-w-xs mx-auto'>
          <DialogHeader>
            ...
            <DialogDescription>{t('confirmCancellationDesc')}</DialogDescription>
          </DialogHeader>
          <div className='py-3 space-y-1'>
            <p className='font-medium'>{t('tableNumber', { number: table.number })}</p>
            <p className='text-sm text-muted-foreground'>
              {format(table.reservation.reservation_time || new Date(), 'EEEE, dd/MM/yyyy')} {t('at')}{' '}
              {format(table.reservation.reservation_time || new Date(), 'HH:mm')}
            </p>
          </div>

...
export const columns: ColumnDef<TableItem>[] = [
  ...
  },
  {
    accessorKey: 'status',
    header: 'Status',
    cell: ({ row }) => <div>{statusBadge(getTableStatus(row.getValue('status')))}</div>
  },
  {
    ...
    >
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Table <span className='bg-foreground text-primary-foreground rounded p-1'>{tableDelete?.number}</span> will

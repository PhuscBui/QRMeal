...
import EditShift from '@/app/manage/shifts/edit-shift'
import AddShift from '@/app/manage/shifts/add-shift'
import ShiftStats from '@/app/manage/shifts/shift-stats'
...
export const shiftsColumns: ColumnDef<ShiftType>[] = [
  {
    accessorKey: 'index',
    header: 'No.',
...
export const requestsColumns: ColumnDef<ShiftType>[] = [
  {
    accessorKey: 'index',
    header: 'No.',
    cell: ({ row }) => <div>{row.index + 1}</div>
  ...
  },
  {
    ...
    header: 'Reason',
    cell: ({ row }) => (
      <div className='max-w-[200px] truncate text-sm'>{row.getValue('reason') || 'No reason provided'}</div>
    )
  },
  {
    ...
    >
      <DialogContent className='sm:max-w-[600px]'>
        <DialogHeader>
          ...
          <DialogDescription>
            Review and approve or reject the shift request from {shift.staff_info?.name}.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className='space-y-4 py-4'>
          <div className='grid grid-cols-2 gap-4'>
            ...
            <div className='space-y-2'>
              <Label className='text-sm font-bold'>Shift Details</Label>
              <div className='text-sm'>
          ...
          {shift.reason && (
            <div className='space-y-2'>
              <Label className='text-sm font-bold'>Employee&apos;s Reason</Label>
            ...
            <form noValidate className='space-y-4' onSubmit={form.handleSubmit(onSubmit)}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Textarea
                      placeholder={
                        reviewAction === 'approve' ? 'Add any notes (optional)' : 'Explain why this request is rejected'
                      }
              ...
              <DialogFooter>
                ...
                <Button type='submit' disabled={reviewShiftRequestMutation.isPending}>
                  {reviewShiftRequestMutation.isPending ? 'Processing...' : 'Confirm'}
                </Button>
              </DialogFooter>
            </form>
          </Form>
    ...
    >
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Shift for{' '}
            <span className='bg-foreground text-primary-foreground rounded p-1'>{shiftDelete?.staff_info?.name}</span>{' '}
            on{' '}
            <span className='bg-foreground text-primary-foreground rounded p-1'>
              {shiftDelete ? new Date(shiftDelete.shift_date).toLocaleDateString('vi-VN') : ''}
            </span>{' '}
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction onClick={deleteShift}>Continue</AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
  )
        ...
        <Tabs defaultValue={activeTab} className='w-full'>
          ...
          <TabsContent value='shifts' className='space-y-4'>
            ...
            <div className='flex items-center justify-between space-x-2 py-4'>
              <div className='text-xs text-muted-foreground py-4 flex-1'>
                Display <strong>{shiftsTable.getPaginationRowModel().rows.length}</strong> of{' '}
                <strong>{shiftsData.length}</strong> shifts
              </div>
              <div>
                <AutoPagination
          ...
          <TabsContent value='requests' className='space-y-4'>
            ...
            <div className='flex items-center justify-end space-x-2 py-4'>
              <div className='text-xs text-muted-foreground py-4 flex-1'>
                Display <strong>{requestsTable.getPaginationRowModel().rows.length}</strong> of{' '}
                <strong>{requestsData.length}</strong> pending requests
              </div>
              <div>
                <AutoPagination

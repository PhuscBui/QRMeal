...
const getColumns = (t: any): ColumnDef<ReviewItem>[] => [
  {
    accessorKey: 'index',
    header: t('no'),
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <div className='max-w-md'>{comment || <span className='text-muted-foreground'>{t('noComment')}</span>}</div>
    }
  },
  {
    ...
    cell: ({ row }) => {
      ...
      try {
        return <div>{format(new Date(date), 'dd/MM/yyyy HH:mm', { locale: vi })}</div>
      } catch {
        return <div>{date}</div>
      }
    }
  ...
  return (
    <div className='w-full'>
      ...
      <div className='flex items-center justify-end space-x-2 py-4'>
        ...
        <div>
          <AutoPagination page={table.getState().pagination.pageIndex + 1} pageSize={Math.ceil(total / PAGE_SIZE)} pathname='/manage/reviews' />
        </div>
      </div>
    </div>
  )

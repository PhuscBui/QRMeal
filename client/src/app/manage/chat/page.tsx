  ...
  return (
    <div className='flex flex-col h-screen w-full p-4 lg:p-6'>
      <div className='mb-2 flex-shrink-0'>
        ...
        <p className='text-sm lg:text-base text-muted-foreground'>Quản lý và trả lời tin nhắn từ khách hàng</p>
      </div>
            ...
            <TabsContent value='active' className='flex-1 overflow-y-auto mt-4 space-y-3 pr-2 min-h-0'>
              ...
              {filteredActiveSessions.length === 0 && (
                <div className='text-center text-muted-foreground py-12'>
                  ...
                  <p className='text-sm font-medium'>Chưa có phiên chat</p>
                  <p className='text-xs mt-1'>Các phiên chat mới sẽ xuất hiện ở đây</p>
                </div>
              )}
            </TabsContent>
            ...
            <TabsContent value='closed' className='flex-1 overflow-y-auto mt-4 space-y-3 pr-2 min-h-0'>
              ...
              {filteredClosedSessions.length === 0 && (
                <div className='text-center text-muted-foreground py-12'>
                  ...
                  <p className='text-sm font-medium'>Chưa có phiên đã đóng</p>
                  <p className='text-xs mt-1'>Các phiên đã kết thúc sẽ xuất hiện ở đây</p>
                </div>
              )}
            </TabsContent>
          </Tabs>
          ...
          {selectedSession ? (
            ...
            <Card className='h-full flex items-center justify-center border shadow-sm bg-gradient-to-br from-muted/20 to-background'>
              <div className='text-center text-muted-foreground p-8'>
                ...
                <h3 className='text-xl font-semibold mb-2 text-foreground'>Chọn một phiên chat</h3>
                <p className='text-sm text-muted-foreground max-w-sm mx-auto'>
                  Chọn phiên chat từ danh sách bên trái để xem chi tiết và trả lời tin nhắn
                </p>
              </div>
            </Card>
          )}
...
function SessionListItem({
  ...
  const getUserInfo = () => {
    if (session.customer_id || session.guest_id) {
      return {
        name: session.user_info?.name || session.user_info?.email || 'Người dùng',
        label: session.customer_id ? 'Khách hàng' : 'Khách',
    ...
    return {
      name: `Ẩn danh #${session.anonymous_id?.slice(-6)}`,
      label: 'Ẩn danh',
...
function ChatDetail({ session }: { session: ChatSession }) {
  ...
  const handleEndSession = () => {
    if (confirm('Bạn có chắc muốn kết thúc phiên chat này?')) {
  ...
  const getUserInfo = () => {
    if (session.customer_id || session.guest_id) {
      return session.user_info?.name || session.user_info?.email || 'Người dùng'
    }
    return `Ẩn danh #${session.anonymous_id?.slice(-6)}`
  }

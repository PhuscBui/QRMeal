...
export default function OrderAnalyticsPage() {
  ...
  const t = useTranslations('orderAnalytics')
    ...
    <div className='container mx-auto px-4 py-6 max-w-6xl'>
      ...
      <div className='mb-8'>
        <div className='flex items-center justify-between'>
          ...
          <Select value={selectedPeriod} onValueChange={setSelectedPeriod}>
            ...
            <SelectContent>
              <SelectItem value='1month'>{t('period1Month')}</SelectItem>
              <SelectItem value='3months'>{t('period3Months')}</SelectItem>
              <SelectItem value='6months'>{t('period6Months')}</SelectItem>
              <SelectItem value='1year'>{t('period1Year')}</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
      ...
      <Tabs value={selectedTab} onValueChange={setSelectedTab} className='space-y-6'>
                  ...
                  {analyticsData.orderTypes.map((type, index) => (
                    <div key={index} className='space-y-2'>
                      <div className='flex justify-between text-sm'>
                        ...
                        <span>
                          {type.count} {tCommon('orders')} ({type.percentage}%)
                        </span>
                      </div>
        ...
        <TabsContent value='preferences' className='space-y-6'>
          <Card>
            <CardHeader>
              <CardTitle>{t('favoriteCategories')}</CardTitle>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
            {analyticsData.achievements.map((achievement, index) => {
              ...
              const achievementData = [
                { title: t('loyalCustomer'), description: t('loyalCustomerDesc') },
                { title: t('foodie'), description: t('foodieDesc') },
                { title: t('vipMember'), description: t('vipMemberDesc') },
                { title: t('saver'), description: t('saverDesc') }
              ][index]
              return (
                <Card
                  key={achievement.id}
                  ...
                  <CardContent className='p-6'>
                    <div className='flex items-start gap-4'>
                      ...
                      <div className='flex-1'>
                        <h4 className='font-medium mb-2'>{achievementData.title}</h4>
                        <p className='text-sm text-muted-foreground mb-3'>{achievementData.description}</p>
                        ...
                        {achievement.unlocked && <Badge className='mt-2 bg-green-500 text-white'>{t('achieved')}</Badge>}
                      </div>
                    </div>
                  </CardContent>
                </Card>

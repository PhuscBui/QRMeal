...
export default function ScanQRPage() {
  ...
  const t = useTranslations('scanQR')
  ...
  const processScannedData = useCallback(
    (data: string) => {
      try {
        ...
        if (table.status === 'Hidden') {
          setError(t('errorTableUnavailable'))
          return
        }

        if (table.status === 'Reserved' && !table.reservation?.is_customer) {
          setError(t('errorTableReserved'))
          return
        }
    ...
    [tables, setError, setScannedData, stopScanning, router, t]
  )
      ...
      {scannedData && (
        <Card className='border-green-200 bg-green-50 dark:bg-green-950/20'>
          <CardContent className='p-6 text-center'>
            ...
            <h2 className='text-xl font-bold text-green-800 dark:text-green-200 mb-2'>{t('scanSuccess')}</h2>
            <p className='text-green-700 dark:text-green-300 mb-4'>{t('redirecting')}</p>
              ...
              {(() => {
                ...
                return (
                  <>
                    ...
                    <div>
                      {t('tableLocation')}: {table.location}
                    </div>
                    <div>
                      {t('tableCapacity')}: {table.capacity} {t('peopleUnit')}
                    </div>
      ...
      {!scannedData && (
        <Card className='mb-6'>
          <CardHeader>
            ...
            <CardDescription>{t('aimCamera')}</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              {isScanning ? (
                ...
                <div className='flex items-center justify-center h-full'>
                  <div className='text-center text-white'>
                    ...
                    <p className='text-lg'>{t('cameraNotStarted')}</p>
                  </div>
                </div>
              )}
        ...
        <Card>
          ...
          <CardContent>
            <div className='space-y-4'>
              <div>
                <Label htmlFor='tableCode'>{t('tableCodeOrURL')}</Label>
                <Input
                  id='tableCode'

...
export default function TableReservationPage() {
  ...
  const handleTableSelect = (table: { _id: string } | null) => {
    if (table) {
      const fullTable = tableList.find((t) => t._id === table._id)
      if (fullTable) {
        setSelectedTable(fullTable)
        setSelectedTableId(fullTable._id)
        setOpen(true)
      }
    } else {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'Available':
        return <Badge className='bg-green-500 hover:bg-green-600'>{tTable('available')}</Badge>
      case 'Reserved':
        return <Badge className='bg-red-500 hover:bg-red-600'>{tTable('reserved')}</Badge>
      default:
        return <Badge variant='outline'>{t('hidden')}</Badge>
    }
  }
  ...
  return (
    <div className='container mx-auto py-8 px-4 md:px-6'>
      <div className='flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4'>
        <div>
          <h1 className='text-3xl font-bold'>{t('tableReservation')}</h1>
          <p className='text-muted-foreground mt-1'>{t('selectTable')}</p>
        </div>
      ...
      <Tabs value={viewMode} onValueChange={setViewMode}>
        <TabsContent value='map' className='mt-0'>
          <FloorMapView tables={filteredTables} onTableSelect={handleTableSelect} selectedTableId={selectedTableId} />
        </TabsContent>
              ...
              <table className='w-full'>
                <thead>
                  <tr className='border-b'>
                    <th className='text-left p-4 font-medium'>{t('table')}</th>
                    <th className='text-left p-4 font-medium'>{t('capacity')}</th>
                    <th className='text-left p-4 font-medium'>{t('location')}</th>
                    <th className='text-left p-4 font-medium'>{tTable('tableStatus')}</th>
                    <th className='text-right p-4 font-medium'>{t('action')}</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredTables.map((table) => (
                    <tr key={table._id} className='border-b hover:bg-muted/50'>
                      ...
                      <td className='p-4 text-right'>
                        <Button
                          ...
                          variant={table.status === 'Available' ? 'default' : 'outline'}
                        >
                          {table.status === 'Available' ? t('reserve') : t('view')}
                        </Button>
                      </td>
                    </tr>
                  ))}
        ...
        <DialogContent className='sm:max-w-[500px]'>
          <DialogHeader>
            <DialogTitle className='text-xl'>
              {t('bookingTable')} {selectedTable?.number}
              <span className='ml-2 text-sm font-normal text-muted-foreground'>
                ({selectedTable?.location}, {selectedTable?.capacity} {t('people')})
              </span>
            </DialogTitle>
          </DialogHeader>

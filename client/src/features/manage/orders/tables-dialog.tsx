  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant='outline'>{showOccupiedTables ? t('selectTable') : t('change')}</Button>
      </DialogTrigger>
      <DialogContent className='sm:max-w-[600px] max-h-full overflow-auto'>
        <DialogHeader>
          <DialogTitle>{showOccupiedTables ? t('selectTableAdditional') : t('selectTableTitle')}</DialogTitle>
          {showOccupiedTables && <p className='text-sm text-gray-600 mt-2'>{t('selectTableDesc')}</p>}
        </DialogHeader>
        <div>
                          ...
                          {row.getVisibleCells().map((cell) => (
                            <TableCell key={cell.id}>
                              ...
                              {cell.column.id === 'status' && isCurrentCustomerTable && (
                                <div className='text-xs text-blue-600 mt-1 font-medium'>{t('currentTable')}</div>
                              )}
                            </TableCell>
                          ))}
                        </TableRow>

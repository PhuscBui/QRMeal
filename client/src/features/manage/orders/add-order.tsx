...
import CustomersDialog from '@/features/manage/orders/customers-dialog'
import { TablesDialog } from '@/features/manage/orders/tables-dialog'
import GuestsDialog from '@/features/manage/orders/guests-dialog'
    ...
    try {
      ...
      if (isCustomer) {
        ...
        if (isNewGuest) {
          ...
          if (!isValid) {
            toast.error(tCommon('error'), {
              description: tCommon('pleaseFillRequired')
            })
            return
          }
          ...
          if (!guestId) {
            toast.error(tCommon('error'), {
              description: t('pleaseSelectGuest')
            })
            return
          }
      ...
      if (orders.length === 0) {
        toast.error(tCommon('error'), {
          description: t('pleaseSelectDish')
        })
        return
      }
      ...
      if (orderType === 'delivery') {
        if (!deliveryInfo.address || !deliveryInfo.receiver_name || !deliveryInfo.receiver_phone) {
          toast.error(tCommon('error'), {
            description: t('pleaseEnterDeliveryInfo')
          })
          return
        }
      }
      ...
      toast.success(tCommon('success'), {
        description: tCommon('createSuccess')
      })
    ...
    >
      ...
      <DialogContent className='sm:max-w-[600px] max-h-screen overflow-auto'>
        ...
        <div className='grid grid-cols-4 items-center justify-items-start gap-4'>
          <Label htmlFor='isCustomer'>{t('customer')}</Label>
        ...
        {isCustomer && (
          <div className='grid grid-cols-4 items-center justify-items-start gap-4'>
            <Label htmlFor='orderType'>{t('orderTypeLabel')}</Label>
        ...
        {!isCustomer && (
          <div className='grid grid-cols-4 items-center justify-items-start gap-4'>
            <Label>{t('orderTypeLabel')}</Label>
            <div className='col-span-3 text-sm text-gray-600'>{t('dineIn')}</div>
          </div>
        )}
        ...
        {isCustomer && orderType === 'delivery' && (
          <div className='space-y-4 p-4 border rounded-lg bg-gray-50'>
            <Label className='text-base font-medium'>{t('delivery')}</Label>
        ...
        {isCustomer && (
          <div className='space-y-4'>
            ...
            {selectedCustomer && (
              <div className='grid grid-cols-4 items-center justify-items-start gap-4'>
                <Label htmlFor='selectedCustomer'>{t('selectCustomer')}</Label>
            ...
            {selectedCustomer && orderType === 'dine-in' && (
              <div className='grid grid-cols-4 items-center justify-items-start gap-4'>
                <Label htmlFor='customerTable'>{t('selectTable')}</Label>
                <div className='col-span-3 w-full space-y-2'>
                  <div className='flex items-center gap-4'>
                    <div>
                      {t('table')} {form.watch('table_number') || tCommon('notSelected')}
                    </div>
                    <TablesDialog
                      onChoose={(table) => {
        ...
        {!isCustomer && !isNewGuest && (
          <div className='space-y-4'>
            ...
            {selectedGuest && (
              <div className='grid grid-cols-4 items-center justify-items-start gap-4'>
                <Label htmlFor='selectedGuest'>{t('selectGuest')}</Label>
                <div className='col-span-3 w-full gap-4 flex items-center'>
                  <div>
                    {selectedGuest.name} (#{selectedGuest._id})
                  </div>
                  <div>
                    {t('table')}: {selectedGuest.table_number}
                  </div>
                </div>
              </div>
            )}
          ...
          >
            <span>
              {createOrderMutation.isPending
                ? t('creatingOrder')
                : `${t('createOrderBtn')} ${orders.length} ${orders.length > 1 ? tCommon('items') : tCommon('item')}`}
            </span>

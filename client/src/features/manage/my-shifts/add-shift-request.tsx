    ...
    <Dialog onOpenChange={setOpen} open={open}>
      <DialogTrigger asChild>
        <Button size='lg' className='gap-2'>
          ...
          <span className='sr-only sm:not-sr-only sm:whitespace-nowrap'>{t('requestShift')}</span>
        </Button>
      </DialogTrigger>
      <DialogContent className='sm:max-w-[500px]'>
        <DialogHeader>
          <DialogTitle>{t('requestNewShift')}</DialogTitle>
          <DialogDescription>
            {t('requestNewShiftDesc')}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form
        ...
        <DialogFooter>
          <Button type='submit' form='add-shift-request-form' disabled={createShiftRequestMutation.isPending}>
            {createShiftRequestMutation.isPending ? t('submitting') : t('submitRequest')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>

...
import EditPromotion from '@/features/manage/promotions/edit-promotion'
import AddPromotion from '@/features/manage/promotions/add-promotion'

type PromotionItem = PromotionListResType['result'][0]
type TranslateFunction = ReturnType<typeof useTranslations<string>>

const getStatusBadge = (status: boolean, t: TranslateFunction) => {
  if (status) {
...
const getColumns = (t: TranslateFunction, tCommon: TranslateFunction): ColumnDef<PromotionItem>[] => [
  {
    accessorKey: 'index',
    header: t('no'),
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      if (conditions.min_spend)
        conditionsList.push(t('minSpendLabel', { amount: formatCurrency(conditions.min_spend) }))
      if (conditions.min_visits) conditionsList.push(t('minVisitsLabel', { count: conditions.min_visits }))
      if (conditions.min_loyalty_points)
        conditionsList.push(t('minPointsLabel', { count: conditions.min_loyalty_points }))

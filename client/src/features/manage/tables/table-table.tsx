...
import AddReservationDialog from '@/features/manage/tables/add-reservation'
import EditTable from '@/features/manage/tables/edit-table'
import AddTable from '@/features/manage/tables/add-table'
import ReservationDetail from '@/features/manage/tables/reservation-detail'
...
const getStatusBadge = (status: string, t: ReturnType<typeof useTranslations>) => {
  if (status === 'Available') {
    return (
      <span className='bg-green-100 text-green-800 rounded-full px-2 py-1 text-xs font-bold'>{t('available')}</span>
    )
  }
  if (status === 'Reserved') {
    ...
    >
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>{t('deleteTableDesc', { number: tableDelete?.number || '' })}</AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>

...
export const usedPromotionController = async (
  ...
  const { customer_id, promotion_id, order_group_ids } = req.body
  const result = await customerPromotionService.usedPromotion(customer_id, promotion_id, order_group_ids)
